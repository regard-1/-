...
document.addEventListener('paste', e => {
  if (state.page !== 'generate' || state.ocrPending || !e.clipboardData) return;
  const target = e.target instanceof HTMLElement ? e.target : null;
  if (!target?.closest('#thread, .composer')) return;
  const file = Array.from(e.clipboardData.files).find(file => file.type.startsWith('image/'));
  if (!file) return;
  e.preventDefault();
  void recognizeScreenshotFile(file);
});
...
document.addEventListener('click', async e => {
  ...
  try {
    ...
    if(a==='add-message'){capture();const t=state.composer.text.trim();if(!t){notify('请输入消息内容');return;}if(state.messages.length>=20){notify('本次对话已达上限，请新建咨询');return;}const role=button.dataset.role;if(!role){notify('请选择添加客户或营养师消息');return;}state.messages.push({role,content:maskText(t)});state.composer.text='';state.composer.role=role==='user'?'assistant':'user';state.editing=null;generator();$('#composer-text')?.focus();return;}
...
try{await enter();}catch{try{const status=await request('/status');state.configured=status.model_configured;state.knowledgeConfigured=!!status.knowledge_configured;renderLogin(status.local_setup);}catch{renderLogin();$('#login-error').className='error';$('#login-error').textContent='服务暂时无法连接，请稍后重试';}}
