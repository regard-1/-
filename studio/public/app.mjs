...
async function loadMaterials() { const items = (await request('/materials')).items; if(state.result?.used_sources.some(ref => ref.id !== 'supplement' && !items.some(m => m.id === ref.id && m.version === ref.version && available(m)))) state.result=null; state.materials = items; for (const id of state.selected) { if (!state.materials.some(m => m.id === id && available(m))) state.selected.delete(id); } }
...
document.addEventListener('click', async e => {
  ...
  try {
    ...
    if(a==='audience'||a==='scene'){capture();state.controller?.abort();state.sequence++;state.pending=false;state.result=null;state.conflicts=[];if(a==='audience'){state.audience=id;for(const rid of state.selected)if(!available(state.materials.find(m=>m.id===rid)))state.selected.delete(rid);}else state.scene=id;generator();return;}
