...
const state = { user: null, csrf: '', page: 'generate', audience: 'anti_aging', scene: 'needs', draft: draft(), composer: { text: '', role: 'assistant' }, editing: null, messages: [], result: null, selected: new Set(), materials: [], filter: 'all', pending: false, error: '', feedback: '', configured: false, sequence: 0, controller: null, conflicts: [] };
let noticeTimer;
...
function clearConsultation() { state.controller?.abort(); state.sequence++; state.pending = false; state.messages = []; state.result = null; state.conflicts = []; state.draft = draft(); state.composer = { text: '', role: 'assistant' }; state.editing = null; state.error = ''; state.feedback = ''; }
